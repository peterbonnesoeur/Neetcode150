    ...
    def isValidSudoku(self, board: List[List[str]]) -> bool:
        ...
        for row_index in range(9):
            for column_index in range(9):
                ...
                if (
                    board[row_index][column_index] in logRows[row_index]
                    or board[row_index][column_index] in logColumns[column_index]
                    or board[row_index][column_index]
                    in logSquares[(row_index // 3, column_index // 3)]
                ):
                    return False

                logColumns[column_index].add(board[row_index][column_index])
                logRows[row_index].add(board[row_index][column_index])
                logSquares[(row_index // 3, column_index // 3)].add(
